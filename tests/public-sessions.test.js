...
test('private/host sessions are excluded from the public list', () => {
  ...
  assert.equal(isPrivateHostSession(row({ has_host: null })), true);
  assert.equal(isPrivateHostSession({ code: 'SS-1', hasHost: true }), true);
  assert.equal(isPrivateHostSession({ code: 'SS-1' }), false, 'missing column is not treated as host');
  assert.equal(isPublicSession(row({ has_host: true }), TODAY), false);
  assert.equal(isPublicSession(row({ has_host: null }), TODAY), false);
  assert.equal(isPublicSession(row({ has_host: false }), TODAY), true);
});
...
test('filterPublicSessions sorts by date then code and drops private/host rows', () => {
...
test('uniqueCourses and uniqueCities stay available as helpers', () => {
  const rows = [
