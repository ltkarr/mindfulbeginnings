...
test('create-session form reserves materials and the dashboard lists materials cues', () => {
  ...
  assert.match(admin, /function matBookBuySentence/);
  assert.match(admin, /Buy '\+p\.shortfall\+' more /);
  assert.match(admin, /MAT_BOOKS_SINCE/);
  assert.match(admin, /Durable kit only/);
  ...
  const renderMat = admin.slice(admin.indexOf('function renderSessionMaterials'), admin.indexOf('function paintSessionMaterialsStatus'));
  assert.match(renderMat, /!l\.consumable/);
  const paint = admin.slice(admin.indexOf('function paintSessionMaterialsStatus'), admin.indexOf('function sessionMaterialsFromForm'));
  assert.match(paint, /matBookBuySentence\(plan\)/);
  assert.doesNotMatch(paint, /confirm\(/);
  const modal = admin.slice(admin.indexOf('function buildCheckoutModal'), admin.indexOf('function coSessChanged'));
  assert.match(modal, /!matEqIsConsumable\(eq\)/);
  const prefill = admin.slice(admin.indexOf('function coPrefillNeeds'), admin.indexOf('function coNeedsHint'));
  assert.doesNotMatch(prefill, /sessionBookNeeds/);
